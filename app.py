...
def render_gap_ticket(result, idx, prewritten, question, conv):
    ...
    with st.expander(label, expanded=prewritten):
        ...
        if st.button("Send gap ticket", key=f"gapsend_{conv['id']}_{idx}", type="primary"):
            ...
            try:
                rag_engine.send_gap_ticket(question, gap, body, missing)
            except Exception:
                pass
            conv["sent"].add(idx)
            st.rerun()
...
with st.sidebar:
    ...
    st.divider()
    st.caption(f"Storage backend: **{store.backend_name()}**")
    ...
    with a2:
        with _top_action("Add Knowledge"):
            ...
            if uploaded is not None and st.button(
                ...
                verb = "Transcribing" if media_ingest.is_media(uploaded.name) else "Ingesting"
...
if prompt:
    ...
    with st.spinner("Synthesizing answer..."):
        result = query_brain(prompt, history=history)
